...
const ORDER: Macro[] = ['calories', 'protein'];
...
function findHeadlineNote(outcome: RoundOutcome): string {
  const entries = ORDER.map((m) => ({
    m,
    abs: Math.abs(outcome.perMacro[m].deltaPct),
    sign: outcome.perMacro[m].deltaPct >= 0 ? 'over' : 'under',
  }));
  entries.sort((a, b) => b.abs - a.abs);
  const worst = entries[0];
  if (!worst || worst.abs < 6) {
    return 'Tightly clustered — you read every macro within 6%.';
  }
